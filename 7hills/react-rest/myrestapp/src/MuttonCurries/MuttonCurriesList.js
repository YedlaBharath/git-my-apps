...
import MuttonCurries from './MuttonCurries'
import {Link} from 'react-router-dom'

export default class MuttonCurriesList extends Component {
    render() {
        return (
                        <>
                <h2 style={{textTransform:"uppercase"}}>7 Hills Mutton Curries Section</h2>
                <Link to="./">
                ...
                <MainContextConsumer>
                    {
                        value=>{
                            // console.log(value)
                            return value.muttonCurryProducts.map((item,index)=>{
                                return <MuttonCurries key={index} item={item}></MuttonCurries>
                            })
                        }
                    }
                </MainContextConsumer>
